...
public class Parser {
    ...
    private static ProgramNode parseSTMT(Scanner s) {
        //Attempts all possible things it could be
    ...
    private static ProgramNode parseIf(Scanner s) {
        if (checkFor("if", s)) {
            List<CondiNode> conds = new ArrayList<>();
     ...
     *
     */
    private static CondiNode parseCond(Scanner s) {
        RelopNode relop = parseRelop(s);
        if (relop == null) {
            CondiNode cond = buildCond(s);
            if (cond == null)
        ...
        NumNode a = parseEXPR(s);
        if (a == null)
            return null;

        require(",", "Missing ','", s);

        NumNode b = parseEXPR(s);
        if (b == null)
            return null;

        require(CLOSEPAREN, "Missing ')'", s);

        final NumNode numA = a, numB = b;
        return new CondiNode() {
     ...
     *
     */
    private static CondiNode buildCond(Scanner s) {
        ...
        if (type != null) {
            ...
            CondiNode a = parseCond(s);
            if (a == null)
                return null;

            require(",", "Missing ','", s);

            CondiNode b = parseCond(s);
            if (b == null)
                return null;
     ...
     * @return
     */
    private static NumNode parseSens(Scanner s) {
        ...
        if (checkFor("barrelFB", s)) {
            ...
            return NumNode.BARREL_FB;
        }
        if (checkFor("fuelLeft", s))
            return NumNode.FUEL_LEFT;
        if (checkFor("oppLR", s))
            return NumNode.OPP_LR;
        if (checkFor("oppFB", s))
            return NumNode.OPP_FB;
        if (checkFor("numBarrels", s))
            return NumNode.NUM_BARRELS;
        if (checkFor("wallDist", s))
            return NumNode.WALL_DIST;
     ...
     * @return
     */
    private static NumNode parseEXPR(Scanner s) {
        NumNode expr = buildEXPR(s);
        if (expr != null)
            return expr;
        expr = parseSens(s);
        if (expr != null)
     ...
     * @return
     */
    private static NumNode buildEXPR(Scanner s) {
        String type = null;
        if (checkFor("add", s))
            type = "add";
...
interface BlockNode extends ProgramNode {
